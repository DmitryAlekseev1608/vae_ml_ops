...
11. Установка pre-commit:
    ...
    pre-commit install
    ```
12. По ТЗ обещали, что сервер mlflow будет развернут на "http://128.0.1.1:8080". Можно в файле command.py поменять в двух строчках с mlflow_server на mlflow_server_test,
    запустив по пункту 16 сам mlflow на localhost:8080 и протестировать работу системы там.
14. Запуск исполнения кода:
    ```bash     
...
15. Требуемая при проверке по ТЗ операция выполняется:
    ```bash
    pre-commit run -a
    ```
16. Запуск тестового сервера ml-flow:
    ```bash
